  ...
  if (msg->status_code == SOUP_STATUS_PROXY_UNAUTHORIZED)
    {
      ...
      if (retrying)
        ...
        soup_auth_authenticate (auth, thread_closure->proxy_user, thread_closure->proxy_password);
    }
}
...
session_thread_set_proxy_cb (ThreadClosure *thread_closure, gpointer data)
{
  ...
  if (proxy_data->proxy_uri)
    g_object_set (thread_closure->session, SOUP_SESSION_PROXY_URI, proxy_data->proxy_uri, NULL);

  if (proxy_data->ntlm_auth)
    soup_session_add_feature_by_type (thread_closure->session, SOUP_TYPE_AUTH_NTLM);
...
ostree_fetcher_session_thread (gpointer data)
{
  ...
  closure->session = soup_session_new_with_options (
